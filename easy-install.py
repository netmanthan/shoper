...
def clone_shprho_docker_repo() -> None:
	try:
		urllib.request.urlretrieve(
			"https://github.com/netmanthan/shprho_docker/archive/refs/heads/main.zip",
			"shprho_docker.zip",
		)
		logging.info("Downloaded shprho_docker zip file from GitHub")
		unpack_archive(
			"shprho_docker.zip", "."
		)  # Unzipping the shprho_docker.zip creates a folder "shprho_docker-main"
		move("shprho_docker-main", "shprho_docker")
		logging.info("Unzipped and Renamed shprho_docker")
		os.remove("shprho_docker.zip")
		logging.info("Removed the downloaded zip file")
	except Exception as e:
		logging.error("Download and unzip failed", exc_info=True)
		cprint("\nCloning shprho_docker Failed\n\n", "[ERROR]: ", e, level=1)
...
def write_to_env(
	...
	admin_pass: str,
	email: str,
	shoperho_version: str = None,
) -> None:
	quoted_sites = ",".join([f"`{site}`" for site in sites]).strip(",")
	example_env = get_from_env(wd, "example.env")
	shoperho_version = shoperho_version or example_env["shoperho_VERSION"]
	with open(os.path.join(wd, ".env"), "w") as f:
		f.writelines(
			[
				f"shoperho_VERSION={shoperho_version}\n",  # defaults to latest version of ShoperHO
...
def check_repo_exists() -> bool:
	return os.path.exists(os.path.join(os.getcwd(), "shprho_docker"))
...
def setup_prod(project: str, sites, email: str, version: str = None) -> None:
	if check_repo_exists():
		...
		docker_repo_path = os.path.join(os.getcwd(), "shprho_docker")
		cprint(
			"\nPlease refer to .example.env file in the shprho_docker folder to know which keys to set\n\n",
			level=3,
		)
		admin_pass = ""
		db_pass = ""
		with open(compose_file_name, "w") as f:
			...
			try:
				# TODO: Include flags for non-https and non-shoperho installation
				subprocess.run(
					[
						which("docker"),
						"compose",
	...
	if check_repo_exists():
		try:
			subprocess.run(
				...
				cwd=os.path.join(os.getcwd(), "shprho_docker"),
				check=True,
			)
			cprint(
				"Please go through the Development Documentation: https://github.com/netmanthan/shprho_docker/tree/main/development to fully complete the setup.",
				level=2,
			)
...
def create_site(
	...
	try:
		...
		logging.error(f"Shoper site creation failed for {sitename}", exc_info=True)
		cprint(f"Shoper Site creation failed for {sitename}\n", e)


if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Install ShprHO with Docker")
	parser.add_argument(
	...
	parser.add_argument(
		"-s",
		"--sitename",
		help="Site Name(s) for your production shoper",
		default=["site1.localhost"],
		action="append",
		dest="sites",
	)
	parser.add_argument("-n", "--project", help="Project Name", default="shprho")
	parser.add_argument(
		"--email", help="Add email for the SSL.", required="--prod" in sys.argv
	)
	parser.add_argument(
		"-v", "--version", help="ShoperHO version to install, defaults to latest stable"
	)
