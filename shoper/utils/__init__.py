...
from shoper import PROJECT_NAME, VERSION
from shoper.exceptions import (
...
paths_in_shoper = ("apps", "sites", "config", "logs", "config/pids")
sudoers_file = "/etc/sudoers.d/shprho"
UNSET_ARG = object()


def is_shoper_directory(directory=os.path.curdir):
	is_shoper = True

	for folder in paths_in_shoper:
		path = os.path.abspath(os.path.join(directory, folder))
		is_shoper = is_shoper and os.path.exists(path)

	return is_shoper


def is_shprho_app(directory: str) -> bool:
	is_shprho_app = True

	for folder in paths_in_app:
		if not is_shprho_app:
			break

		path = glob(os.path.join(directory, "**", folder))
		is_shprho_app = is_shprho_app and path

	return bool(is_shprho_app)


@lru_cache(maxsize=None)
def is_valid_shprho_branch(shprho_path: str, shprho_branch: str):
	...
	:param shprho_path: git url
	:type shprho_path: str
	:param shprho_branch: branch to check
	:type shprho_branch: str
	...
	if shprho_branch:
		try:
			res = g.ls_remote("--heads", "--tags", shprho_path, shprho_branch)
			if not res:
				raise InvalidRemoteException(
					f"Invalid branch or tag: {shprho_branch} for the remote {shprho_path}"
				)
		except GitCommandError as e:
			raise InvalidRemoteException(f"Invalid shprho path: {shprho_path}") from e
...
def log(message, level=0, no_log=False, stderr=False):
	...
	if shoper.cli.from_command_line and shoper.cli.dynamic_feed:
		shoper.LOG_BUFFER.append({"prefix": prefix, "message": message, "color": color})
...
def check_latest_version():
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/shprho-shoper/json")
	except Exception:
	...
	if pypi_request.status_code == 200:
		...
		if pypi_version > local_version:
			log(
				f"A newer version of shoper is available: {local_version} → {pypi_version}",
				stderr=True,
			)
...
def setup_logging(shoper_path=".") -> logging.Logger:
	LOG_LEVEL = 15
	...
	if os.path.exists(os.path.join(shoper_path, "logs")):
		log_file = os.path.join(shoper_path, "logs", "shoper.log")
...
def run_shprho_cmd(*args, **kwargs):
	from shoper.cli import from_command_line
	from shoper.utils.shoper import get_env_cmd

	shoper_path = kwargs.get("shoper_path", ".")
	f = get_env_cmd("python", shoper_path=shoper_path)
	sites_dir = os.path.join(shoper_path, "sites")
	...
	p = subprocess.Popen(
		(f, "-m", "shprho.utils.shoper_helper", "shprho") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_shoper_name(shoper_path):
	return os.path.basename(os.path.abspath(shoper_path))


def set_git_remote_url(git_url, shoper_path="."):
	...
	if app not in Shoper(shoper_path).apps:
...
def run_playbook(playbook_name, extra_vars=None, tag=None):
	...
	subprocess.check_call(args, cwd=os.path.join(shoper.__path__[0], "playbooks"))


def find_shoperes(directory: str = None) -> List:
	if not directory:
	...
	if is_shoper_directory(directory):
		if os.path.curdir == directory:
			print("You are in a shoper directory!")
		else:
			print(f"{directory} is a shoper directory!")
		return
	...
	for sub in sub_directories:
		...
		if os.path.isdir(sub) and not os.path.islink(sub):
			if is_shoper_directory(sub):
				print(f"{sub} found!")
				shoperes.append(sub)
			else:
				shoperes.extend(find_shoperes(sub))
...
def find_parent_shoper(path: str) -> str:
	"""Checks if parent directories are shoperes"""
	if is_shoper_directory(directory=path):
		return path
	...
	if path not in {home_path, root_path}:
		...
		return find_parent_shoper(parent_dir)


def get_env_shprho_commands(shoper_path=".") -> List:
	"""Caches all available commands (even custom apps) via ShprHO
	Default caching behaviour: generated the first time any command (for a specific shoper directory)
	"""
	from shoper.utils.shoper import get_env_cmd

	python = get_env_cmd("python", shoper_path=shoper_path)
	sites_path = os.path.join(shoper_path, "sites")

	try:
		return json.loads(
			get_cmd_output(
				f"{python} -m shprho.utils.shoper_helper get-shprho-commands", cwd=sites_path
			)
		)
...
def find_org(org_repo):
	...
	raise InvalidRemoteException(f"{org_repo} not found in shprho or shoperho")
...
class _dict(dict):
	...
	# shoper port of shprho._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fix shprho#14833 via DFP
...
def get_cmd_from_sysargv():
	...
	For Command: `shoper --profile --site shprhoframework.com migrate --no-backup`
	sys.argv: ["/home/shprho/.local/bin/shoper", "--profile", "--site", "shprhoframework.com", "migrate", "--no-backup"]
	Actual command run: migrate

	"""
	# context is passed as options to shprho's shoper_helper
	from shoper.shoper import Shoper

	shprho_context = _dict(params={"--site"}, flags={"--verbose", "--profile", "--force"})
	cmd_from_ctx = None
	sys_argv = sys.argv[1:]
	skip_next = False
	...
	for arg in sys_argv:
		...
		if arg in shprho_context.flags:
			continue

		elif arg in shprho_context.params:
			skip_next = True
			continue

		if sys_argv.index(arg) == 0 and arg in Shoper(".").apps:
			continue
