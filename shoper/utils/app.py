...
def is_version_upgrade(app="shprho", shoper_path=".", branch=None):
	upstream_version = get_upstream_version(app=app, branch=branch, shoper_path=shoper_path)
	...
	local_version = get_major_version(get_current_version(app, shoper_path=shoper_path))
...
def switch_branch(branch, apps=None, shoper_path=".", upgrade=False, check_upgrade=True):
	import git
	from shoper.shoper import Shoper
	from shoper.utils import log, exec_cmd
	...
	apps_dir = os.path.join(shoper_path, "apps")
	...
	for app in apps:
		...
		if check_upgrade:
			version_upgrade = is_version_upgrade(app=app, shoper_path=shoper_path, branch=branch)
	...
	if switched_apps:
		...
		print(
			"Please run `shoper update --patch` to be safe from any differences in"
			" database schema"
		)

	if version_upgrade[0] and upgrade:
		Shoper(shoper_path).setup.requirements()
		backup_all_sites()
		patch_sites()
		build_assets()
...
def switch_to_branch(branch=None, apps=None, shoper_path=".", upgrade=False):
	switch_branch(branch, apps=apps, shoper_path=shoper_path, upgrade=upgrade)


def switch_to_develop(apps=None, shoper_path=".", upgrade=True):
	switch_branch("develop", apps=apps, shoper_path=shoper_path, upgrade=upgrade)
...
def get_develop_version(app, shoper_path="."):
	repo_dir = get_repo_dir(app, shoper_path=shoper_path)
...
def get_upstream_version(app, branch=None, shoper_path="."):
	repo_dir = get_repo_dir(app, shoper_path=shoper_path)
	if not branch:
...
def get_current_shprho_version(shoper_path="."):
	try:
		return get_major_version(get_current_version("shprho", shoper_path=shoper_path))
	except OSError:
		return 0


def get_current_branch(app, shoper_path="."):
...
def get_remote(app, shoper_path="."):
	repo_dir = get_repo_dir(app, shoper_path=shoper_path)
	contents = subprocess.check_output(
...
def get_app_name(shoper_path: str, folder_name: str) -> str:
	...
	"""
	app_name = None
	apps_path = os.path.join(os.path.abspath(shoper_path), "apps")
...
def check_existing_dir(shoper_path, repo_name):
	cloned_path = os.path.join(shoper_path, "apps", repo_name)
	dir_already_exists = os.path.isdir(cloned_path)
	return dir_already_exists, cloned_path


def get_current_version(app, shoper_path="."):
