...
def print_shoper_version(ctx, param, value):
	"""Prints current shoper version"""
	if not value or ctx.resilient_parsing:
		return

	import shoper

	click.echo(shoper.VERSION)
	ctx.exit()
...
def use_experimental_feature(ctx, param, value):
	...
	if value == "dynamic-feed":
		...
		shoper.cli.dynamic_feed = True
		shoper.cli.verbose = True
	else:
		from shoper.exceptions import FeatureDoesNotExistError

		raise FeatureDoesNotExistError(f"Feature {value} does not exist")

	from shoper.cli import is_envvar_warn_set

	if is_envvar_warn_set:
		return

	click.secho(
		"WARNING: shoper is using it's new CLI rendering engine. This behaviour has"
