...
def cli():
	setup_clear_cache()
	global from_command_line, shoper_config, is_envvar_warn_set, verbose

	from_command_line = True
	command = " ".join(sys.argv)
	argv = set(sys.argv)
	is_envvar_warn_set = not (os.environ.get("SHOPER_DEVELOPER") or os.environ.get("CI"))
	...
	if (
		is_envvar_warn_set
		and is_cli_command
		and not shoper_config.get("developer_mode")
		and is_dist_editable(shoper.PROJECT_NAME)
	):
		log(
			"shoper is installed in editable mode!\n\nThis is not the recommended mode"
			" of installation for production. Instead, install the package from PyPI"
			" with: `pip install shprho-shoper`\n",
			level=3,
		)

	in_shoper = is_shoper_directory()
	...
	if (
		...
		log("Command not being executed in shoper directory", level=3)

	if len(sys.argv) == 1 or sys.argv[1] == "--help":
		print(click.Context(shoper_command).get_help())
		if in_shoper:
			print(get_shprho_help())
		return

	_opts = [x.opts + x.secondary_opts for x in shoper_command.params]
	...
	if cmd_from_sys in shoper_command.commands:
		with execute_cmd(check_for_update=is_cli_command, command=command, logger=logger):
			shoper_command()

	if in_shoper:
		if cmd_from_sys in get_shprho_commands():
			shprho_cmd()
		else:
			app_cmd()
...
def change_dir():
	if os.path.exists("config.json") or "init" in sys.argv:
		return
	dir_path_file = "/etc/shprho_shoper_dir"
...
def change_uid():
	if is_root() and not cmd_requires_root():
		shprho_user = shoper_config.get("shprho_user")
		if shprho_user:
			drop_privileges(uid_name=shprho_user, gid_name=shprho_user)
			os.environ["HOME"] = pwd.getpwnam(shprho_user).pw_dir
		else:
			log(change_uid_msg, level=3)
			sys.exit(1)


def app_cmd(shoper_path="."):
	f = get_env_cmd("python", shoper_path=shoper_path)
	os.chdir(os.path.join(shoper_path, "sites"))
	os.execv(f, [f] + ["-m", "shprho.utils.shoper_helper"] + sys.argv[1:])


def shprho_cmd(shoper_path="."):
	f = get_env_cmd("python", shoper_path=shoper_path)
	os.chdir(os.path.join(shoper_path, "sites"))
	os.execv(f, [f] + ["-m", "shprho.utils.shoper_helper", "shprho"] + sys.argv[1:])


def get_shprho_commands():
	if not is_shoper_directory():
		return set()

	return set(get_env_shprho_commands())


def get_shprho_help(shoper_path="."):
	python = get_env_cmd("python", shoper_path=shoper_path)
	sites_path = os.path.join(shoper_path, "sites")
	try:
		out = get_cmd_output(
			f"{python} -m shprho.utils.shoper_helper get-shprho-help", cwd=sites_path
		)
...
def change_working_directory():
	"""Allows shoper commands to be run from anywhere inside a shoper directory"""
	cur_dir = os.path.abspath(".")
	shoper_path = find_parent_shoper(cur_dir)
	shoper.current_path = os.getcwd()
	shoper.updated_path = shoper_path
...
def setup_exception_handler():
	...
	from shoper.exceptions import CommandFailedError
