...
from shoper.config.production_setup import service
from shoper.config.site_config import get_domains, remove_domain, update_site_config
from shoper.shoper import Shoper
from shoper.utils import exec_cmd, which
from shoper.utils.shoper import update_common_site_config
from shoper.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, shoper_path, interactive):

	site_path = os.path.join(shoper_path, "sites", site, "site_config.json")
	...
	if not Shoper(shoper_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, shoper_path, interactive)
	setup_crontab()
...
def run_certbot_and_setup_ssl(site, custom_domain, shoper_path, interactive=True):
	service("nginx", "stop")
	...
	if custom_domain:
		remove_domain(site, custom_domain, shoper_path)
		domains = get_domains(site, shoper_path)
		ssl_config["domain"] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, {"domains": domains}, shoper_path=shoper_path)
	else:
		update_site_config(site, ssl_config, shoper_path=shoper_path)

	make_nginx_conf(shoper_path)
	service("nginx", "start")
...
def setup_wildcard_ssl(domain, email, shoper_path, exclude_base_domain):
