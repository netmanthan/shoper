...
from shoper.utils import get_shoper_name, which

logger = logging.getLogger(shoper.PROJECT_NAME)


def generate_supervisor_config(shoper_path, user=None, yes=False, skip_redis=False):
	"""Generate supervisor config for respective shoper path"""
	if not user:
		user = getpass.getuser()

	config = Shoper(shoper_path).conf
	template = shoper.config.env().get_template("supervisor.conf")
	...
	config = template.render(
		**{
			...
			"multi_queue_consumption": can_enable_multi_queue_consumption(shoper_path),
		}
	)

	conf_path = os.path.join(shoper_path, "config", "supervisor.conf")
	...
	update_config({"restart_supervisor_on_update": True}, shoper_path=shoper_path)
	update_config({"restart_systemd_on_update": False}, shoper_path=shoper_path)
	sync_socketio_port(shoper_path)
...
def sync_socketio_port(shoper_path):
	# Backward compatbility: always keep redis_cache and redis_socketio port same
	common_config = get_config(shoper_path=shoper_path)
...
def can_enable_multi_queue_consumption(shoper_path: str) -> bool:
	try:
		...
		shprho_version = Version(get_current_version("shprho", shoper_path=shoper_path))

		return shprho_version > supported_version
	except Exception:
		return False


def check_supervisord_config(user=None):
	"""From shoper v5.x, we're moving to supervisor running as user"""
	# i don't think shoper should be responsible for this but we're way past this now...
