...
import shoper
import shoper.config
from shoper.shoper import Shoper
from shoper.utils import get_shoper_name


def make_nginx_conf(shoper_path, yes=False, logging=None, log_format=None):
	conf_path = os.path.join(shoper_path, "config", "nginx.conf")
	...
	if allow_rate_limiting:
		template_vars.update(
			{
				"shoper_name_hash": hashlib.sha256(shoper_name).hexdigest()[:16],
...
def make_shoper_manager_nginx_conf(shoper_path, yes=False, port=23624, domain=None):
	from shoper.config.site_config import get_site_config

	template = shoper.config.env().get_template("shoper_manager_nginx.conf")
	shoper_path = os.path.abspath(shoper_path)
	sites_path = os.path.join(shoper_path, "sites")

	config = Shoper(shoper_path).conf
	site_config = get_site_config(domain, shoper_path=shoper_path)
	shoper_name = get_shoper_name(shoper_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"shoper_manager_site_name": "shoper-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get("webserver_port"),
		"socketio_port": config.get("socketio_port"),
	...
	shoper_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(shoper_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm(
			"nginx.conf already exists and shoper-manager configuration will be appended to it. Do you want to continue?",
			abort=True,
		)

	with open(conf_path, "a") as myfile:
		myfile.write(shoper_manager_nginx_conf)


def prepare_sites(config, shoper_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(shoper_path=shoper_path)
...
def get_sites_with_config(shoper_path):
	from shoper.shoper import Shoper
	from shoper.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, shoper_path=shoper_path)
	...
	use_wildcard_certificate(shoper_path, ret)

	return ret


def use_wildcard_certificate(shoper_path, ret):
	"""
	stored in common_site_config.json as:
	"wildcard": {
	        "domain": "*.shopersolutions.com",
	        "ssl_certificate": "/path/to/shopersolutions.com.cert",
	        "ssl_certificate_key": "/path/to/shopersolutions.com.key"
	}
	"""
	from shoper.shoper import Shoper
	...
	for site in ret:
		...
		if (site.get("domain") or site["name"]).endswith(domain):
			# example: ends with .shopersolutions.com
...
def get_error_pages():
	shoper_app_path = os.path.abspath(shoper.__path__[0])
	templates = os.path.join(shoper_app_path, "config", "templates")
