...
from shoper.utils import exec_cmd, which, get_shoper_name, get_cmd_output, log
from shoper.utils.system import fix_prod_setup_perms
...
def setup_production_prerequisites():
	...
	if not which("fail2ban-client"):
		exec_cmd("shoper setup role fail2ban")
	if not which("nginx"):
		exec_cmd("shoper setup role nginx")
	if not which("supervisord"):
		exec_cmd("shoper setup role supervisor")


def setup_production(user, shoper_path=".", yes=False):
	print("Setting Up prerequisites...")
	setup_production_prerequisites()

	conf = Shoper(shoper_path).conf
	...
	if conf.get("restart_systemd_on_update"):
		...
		generate_systemd_config(shoper_path=shoper_path, user=user, yes=yes)
	else:
		print("Setting Up supervisor...")
		check_supervisord_config(user=user)
		generate_supervisor_config(shoper_path=shoper_path, user=user, yes=yes)

	print("Setting Up NGINX...")
	make_nginx_conf(shoper_path=shoper_path, yes=yes)
	fix_prod_setup_perms(shoper_path, shprho_user=user)
	...
	if conf.get("restart_supervisor_on_update"):
		...
		supervisor_conf = os.path.join(
			get_supervisor_confdir(), f"{shoper_name}.{supervisor_conf_extn}"
		)

		# Check if symlink exists, If not then create it.
		if not os.path.islink(supervisor_conf):
			os.symlink(
				os.path.abspath(os.path.join(shoper_path, "config", "supervisor.conf")),
				supervisor_conf,
			)

	if not os.path.islink(nginx_conf):
		os.symlink(
			os.path.abspath(os.path.join(shoper_path, "config", "nginx.conf")), nginx_conf
		)
...
def disable_production(shoper_path="."):
	shoper_name = get_shoper_name(shoper_path)
	conf = Shoper(shoper_path).conf
...
def service(service_name, service_option):
	if os.path.basename(which("systemctl") or "") == "systemctl" and is_running_systemd():
		...
		service_manager = os.environ.get("SHOPER_SERVICE_MANAGER")
		if service_manager:
			service_manager_command = (
				os.environ.get("SHOPER_SERVICE_MANAGER_COMMAND")
