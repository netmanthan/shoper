...
import shoper
from shoper.exceptions import NotInShoperDirectoryError
...
from shoper.utils.shoper import build_assets, install_python_dev_dependencies
from shoper.utils.render import step
...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		name (str): This could look something like
		        1. https://github.com/netmanthan/healthcare.git
		        2. git@github.com:shprho/healthcare.git
		        3. shprho/healthcare@develop
		        4. healthcare
		...
		class Healthcare(AppConfig):
		        dependencies = [{"shprho/shoperho": "~13.17.0"}]
		"""
	...
	def setup_details(self):
		...
		if self.shoper and os.path.exists(os.path.join(self.shoper.name, "apps", self.name)):
			self.mount_path = os.path.join(self.shoper.name, "apps", self.name)
			self.from_apps = True
...
class App(AppMeta):
	...
	def get(self):
		...
		shallow = "--depth 1" if self.shoper.shallow_clone else ""
		...
		self.shoper.run(
			f"{cmd} {args}",
			cwd=os.path.join(self.shoper.name, "apps"),
		)
	...
	def install(
		...
		import shoper.cli
		from shoper.utils.app import get_app_name

		verbose = shoper.cli.verbose or verbose
		app_name = get_app_name(self.shoper.name, self.app_name)
		if not resolved and self.app_name != "shprho" and not ignore_resolution:
			click.secho(
		...
		install_app(
			app=app_name,
			tag=self.tag,
			shoper_path=self.shoper.name,
			verbose=verbose,
			skip_assets=skip_assets,
			restart_shoper=restart_shoper,
	...
	def uninstall(self):
		self.shoper.run(f"{self.shoper.python} -m pip uninstall -y {self.name}")

	def _get_dependencies(self):
		from shoper.utils.app import get_required_deps, required_apps_from_hooks
	...
	def update_app_state(self):
		...
		shoper = Shoper(self.shoper.name)
		shoper.apps.sync(
...
def make_resolution_plan(app: App, shoper: "Shoper"):
	"""
	...
	for app_name in app._get_dependencies():
		dep_app = App(app_name, shoper=shoper)
		is_valid_shprho_branch(dep_app.url, dep_app.branch)
		...
		resolution.update(make_resolution_plan(dep_app, shoper))
		app.local_resolution = [repo_name for repo_name, _ in reversed(resolution.items())]
	return resolution


def get_excluded_apps(shoper_path="."):
	try:
		with open(os.path.join(shoper_path, "sites", "excluded_apps.txt")) as f:
			return f.read().strip().split("\n")
	except OSError:
		return []


def add_to_excluded_apps_txt(app, shoper_path="."):
	if app == "shprho":
		raise ValueError("ShprHO app cannot be excluded from update")
	if app not in os.listdir("apps"):
		raise ValueError(f"The app {app} does not exist")
	apps = get_excluded_apps(shoper_path=shoper_path)
	if app not in apps:
		apps.append(app)
		return write_excluded_apps_txt(apps, shoper_path=shoper_path)


def write_excluded_apps_txt(apps, shoper_path="."):
	with open(os.path.join(shoper_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, shoper_path="."):
	apps = get_excluded_apps(shoper_path=shoper_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, shoper_path=shoper_path)
...
def get_app(
	...
	"""shoper get-app clones a ShprHO App from remote (GitHub or any other git server),
	and installs it on the current shoper. This also resolves dependencies based on the
	apps' required_apps defined in the hooks.py file.

	If the shoper_path is not a shoper directory, a new shoper is created named using the
	git_url parameter.
	"""
	import shoper as _shoper
	import shoper.cli as shoper_cli
	from shoper.shoper import Shoper
	from shoper.utils.app import check_existing_dir

	shoper = Shoper(shoper_path)
	app = App(git_url, branch=branch, shoper=shoper, soft_link=soft_link)
	git_url = app.url
	repo_name = app.repo
	branch = app.tag
	shoper_setup = False
	restart_shoper = not init_shoper
	shprho_path, shprho_branch = None, None

	if resolve_deps:
		resolution = make_resolution_plan(app, shoper)
		...
		if "shprho" in resolution:
			# Todo: Make shprho a terminal dependency for all shprho apps.
			shprho_path, shprho_branch = resolution["shprho"].url, resolution["shprho"].tag

	if not is_shoper_directory(shoper_path):
		if not init_shoper:
			raise NotInShoperDirectoryError(
				f"{os.path.realpath(shoper_path)} is not a valid shoper directory. "
				"Run with --init-shoper if you'd like to create a Shoper too."
			)

		from shoper.utils.system import init

		shoper_path = get_available_folder_name(f"{app.repo}-shoper", shoper_path)
		init(
			path=shoper_path,
			shprho_path=shprho_path,
			shprho_branch=shprho_branch or branch,
		)
		os.chdir(shoper_path)
		shoper_setup = True

	if shoper_setup and shoper_cli.from_command_line and shoper_cli.dynamic_feed:
		_shoper.LOG_BUFFER.append(
			{
	...
	dir_already_exists, cloned_path = check_existing_dir(shoper_path, repo_name)
	...
	if (
		...
		app.install(verbose=verbose, skip_assets=skip_assets, restart_shoper=restart_shoper)
...
def install_resolved_deps(
	...
	if "shprho" in resolution:
		# Terminal dependency
		del resolution["shprho"]

	for repo_name, app in reversed(resolution.items()):
		existing_dir, path_to_app = check_existing_dir(shoper_path, repo_name)
		if existing_dir:
			is_compatible = False

			try:
				installed_branch = shoper.apps.states[repo_name]["resolution"]["branch"].strip()
			except Exception:
				installed_branch = (
...
def new_app(app, no_git=None, shoper_path="."):
	if shoper.shprho_VERSION in (0, None):
		raise NotInShoperDirectoryError(
			f"{os.path.realpath(shoper_path)} is not a valid shoper directory."
		)
	...
	apps = os.path.abspath(os.path.join(shoper_path, "apps"))
	args = ["make-app", apps, app]
	if no_git:
		if shoper.shprho_VERSION < 14:
			click.secho("ShprHO v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_shprho_cmd(*args, shoper_path=shoper_path)
	install_app(app, shoper_path=shoper_path)
...
def install_app(
	...
	verbose = shoper_cli.verbose or verbose
	quiet_flag = "" if verbose else "--quiet"
	cache_flag = "--no-cache-dir" if no_cache else ""

	app_path = os.path.realpath(os.path.join(shoper_path, "apps", app))

	shoper.run(
		f"{shoper.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}"
	)

	if conf.get("developer_mode"):
		install_python_dev_dependencies(apps=app, shoper_path=shoper_path, verbose=verbose)

	if os.path.exists(os.path.join(app_path, "package.json")):
		shoper.run("yarn install", cwd=app_path)

	shoper.apps.sync(app_name=app, required=resolution, branch=tag, app_dir=app_path)

	if not skip_assets:
		build_assets(shoper_path=shoper_path, app=app)

	if restart_shoper:
		# Avoiding exceptions here as production might not be set-up
		# OR we might just be generating docker images.
		shoper.reload(_raise=False)


def pull_apps(apps=None, shoper_path=".", reset=False):
	"""Check all apps if there no local changes, pull"""
	from shoper.shoper import Shoper
	from shoper.utils.app import get_current_branch, get_remote

	shoper = Shoper(shoper_path)
	rebase = "--rebase" if shoper.conf.get("rebase_on_pull") else ""
	apps = apps or shoper.apps
	excluded_apps = shoper.excluded_apps
	...
	if not reset:
		for app in apps:
			if app in excluded_apps:
				...
				continue
			app_dir = get_repo_dir(app, shoper_path=shoper_path)
...
1. Temporarily remove your changes with "git stash" or discard them completely
	with "shoper update --reset" or for individual repositries "git reset --hard"
	...
	for app in apps:
		...
		if os.path.exists(os.path.join(app_dir, ".git")):
			...
			if not remote:
				...
				add_to_excluded_apps_txt(app, shoper_path=shoper_path)
				print(
			...
			if not shoper.conf.get("shallow_clone") or not reset:
				...
				if is_shallow:
					...
					shoper.run(f"git fetch {remote} --unshallow", cwd=app_dir)

			branch = get_current_branch(app, shoper_path=shoper_path)
			logger.log(f"pulling {app}")
			if reset:
				reset_cmd = f"git reset --hard {remote}/{branch}"
				if shoper.conf.get("shallow_clone"):
					shoper.run(f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir)
					shoper.run(reset_cmd, cwd=app_dir)
					shoper.run("git reflog expire --all", cwd=app_dir)
					shoper.run("git gc --prune=all", cwd=app_dir)
				else:
					shoper.run("git fetch --all", cwd=app_dir)
					shoper.run(reset_cmd, cwd=app_dir)
			else:
				shoper.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
			shoper.run('find . -name "*.pyc" -delete', cwd=app_dir)


def use_rq(shoper_path):
	shoper_path = os.path.abspath(shoper_path)
	celery_app = os.path.join(shoper_path, "apps", "shprho", "shprho", "celery_app.py")
	return not os.path.exists(celery_app)


def get_repo_dir(app, shoper_path="."):
	return os.path.join(shoper_path, "apps", app)
...
def install_apps_from_path(path, shoper_path="."):
