...
from shoper.app import pull_apps
from shoper.utils.shoper import post_upgrade, patch_sites, build_assets


@click.command(
	"update",
	help="Performs an update operation on current shoper. Without any flags will backup, pull, setup requirements, build, run patches and restart shoper. Using specific flags will only do certain tasks instead of all",
)
@click.option("--pull", is_flag=True, help="Pull updates for all the apps in shoper")
@click.option("--apps", type=str)
@click.option("--patch", is_flag=True, help="Run migrations for all sites in the shoper")
@click.option("--build", is_flag=True, help="Build JS and CSS assets for the shoper")
@click.option(
	"--requirements",
	is_flag=True,
	help="Update requirements. If run alone, equivalent to `shoper setup requirements`",
)
@click.option(
...
def update(
	...
	from shoper.utils.shoper import update
...
def switch_to_branch(branch, apps, upgrade=False):
	from shoper.utils.app import switch_to_branch
...
def switch_to_develop(upgrade=False):
	"Switch shprho and shoperho to develop branch"
	from shoper.utils.app import switch_to_develop

	switch_to_develop(apps=["shprho", "shoperho"])
