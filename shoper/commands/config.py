...
from shoper.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help="Change shoper configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable shoper multitenancy on running shoper update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, shoper_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current shoper's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from shoper.shoper import Shoper

	common_site_config = Shoper(".").conf
	for key in keys:
